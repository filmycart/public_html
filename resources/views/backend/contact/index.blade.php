                            ...
                            <tr>
                                <td>
                                    <div class="form-group">
                                        <div class="aiz-checkbox-inline">
                                            <label class="aiz-checkbox">
                                                <input type="checkbox" class="check-one" name="id[]" value="{{$contact->id}}">
                                                <span class="aiz-square-check"></span>
                                            </label>
                                        </div>
                                    </div>
                                </td>
                                <td>@if($contact->banned == 1) <i class="fa fa-ban text-danger" aria-hidden="true"></i> @endif {{$contact->name}}</td>
                                <td>{{$contact->email}}</td>
                                <td>{{$contact->phone}}</td>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>{{translate('Do you really want to ban this Contact?')}}</p>
            </div>
...
<div class="modal fade" id="confirm-unban">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>{{translate('Do you really want to unban this Contact?')}}</p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-light" data-dismiss="modal">{{translate('Cancel')}}</button>
...
@section('script')
    <script type="text/javascript">
        ...
        function bulk_delete() {
            var data = new FormData($('#sort_contacts')[0]);
            $.ajax({
                headers: {
